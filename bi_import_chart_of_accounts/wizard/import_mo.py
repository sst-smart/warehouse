...
class ImportMo(models.TransientModel):
    _name = "import.mo"
    _description = "Import MO"
...
# ------------------------------------------------------------
    ...
    def create_chart_accounts(self, values):
        ...
        product_search_mo = product_obj.search([
            ('name', '=', values.get('mo_id'))
        ], limit=1)
        if product_search:
            product_search.mo_link_product_id = product_search_mo.id
        else:
            pass
